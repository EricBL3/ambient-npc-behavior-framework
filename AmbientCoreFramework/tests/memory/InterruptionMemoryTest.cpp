...
{
    InterruptionMemory memory(5, 10, 15,
        20, 456);
...
TEST(InterruptionMemoryTest, ConstructorHandlesNullEntity) {
    InterruptionMemory memory(5, 10, 15,
        -1, 456);  // -1 indicates null entity

    EXPECT_EQ(-1, memory.GetInterruptedTargetEntityId());
}

TEST(InterruptionMemoryTest, RejectsNegativeActionId) {
    EXPECT_THROW(InterruptionMemory(-1, 10, 15,
        20, 100), std::invalid_argument);
}

TEST(InterruptionMemoryTest, RejectsNegativeSequenceId) {
    EXPECT_THROW(InterruptionMemory(5, -1, 15,
        20, 100), std::invalid_argument);
}

TEST(InterruptionMemoryTest, RejectsNegativeNodeId) {
    EXPECT_THROW(InterruptionMemory(5, 10, -1,
        20, 100), std::invalid_argument);
}

TEST(InterruptionMemoryTest, RejectsInvalidEntityIds) {
    EXPECT_THROW(InterruptionMemory(5, 10, 15,
        -2, 100), std::invalid_argument);  // -2 not allowed
    EXPECT_THROW(InterruptionMemory(5, 10, 15,
        -999, 100), std::invalid_argument); // Other negatives not allowed
}
...
TEST(InterruptionMemoryTest, MatchesRequiresAllThreeKeys) {
    InterruptionMemory base_memory(5, 10, 15,
        20, 100);

    InterruptionMemory same_context(5, 10, 15,
        25, 200);  // Different entity, same context
...
TEST(InterruptionMemoryTest, DoesNotMatchDifferentAction) {
    InterruptionMemory memory1(5, 10, 15,
        20, 100);

    InterruptionMemory memory2(7, 10, 15,
        20, 100);  // Different action
...
TEST(InterruptionMemoryTest, DoesNotMatchDifferentSequence) {
    ...
    InterruptionMemory memory2(5, 12, 15,
        20, 100);  // Different sequence
...
TEST(InterruptionMemoryTest, DoesNotMatchDifferentNode) {
    ...
    InterruptionMemory memory2(5, 10, 17,
        20, 100);  // Different node
...
TEST(InterruptionMemoryTest, EntityIdDoesNotAffectMatching) {
    ...
    InterruptionMemory memory2(5, 10, 15,
        25, 200);   // Entity 25

    InterruptionMemory memory3(5, 10, 15,
        -1, 300);   // Null entity
